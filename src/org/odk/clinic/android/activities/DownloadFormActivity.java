...
//            showDialog(FORMLIST_DIALOG);
    ...
    private void downloadAllForms()
    {
        ...
        argsList.add(url.toString());
        for (int i=0; i < mForms.size(); i++) {
        	arg = mForms.get(i).getFormId() + "";
        	if (!doesFormExist(arg)) {
        		argsList.add(arg);
        		System.out.println("Will try to download form "+ arg);
        	} else {
        		System.out.println("Form "+ arg + " already downloaded");
        	}
        }
        
        mDownloadTask.execute(argsList.toArray(new String[argsList.size()]));
    }
    ...
    private void getForms() {
        ...
        if (c != null && c.getCount() >= 0) {
            ...
            int pathIndex = c
                    .getColumnIndex(ClinicAdapter.KEY_PATH);
            ...
            if (c.getCount() > 0) {
                do {
                    ...
                    mForms.add(f);
                    System.out.println("Adding form " + f.getName() + " " + f.getFormId());
                } while (c.moveToNext());
            }
        }
    ...
    private AlertDialog createFormListDialog() {
        ...
        if (!mForms.isEmpty()) {
            ...
            builder.setMessage(getString(R.string.no_forms));
        }
